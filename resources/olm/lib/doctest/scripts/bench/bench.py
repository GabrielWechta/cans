...
def addCommonFlags(parser):
    ...
    parser.add_argument(
        "--catch", action="store_true", help="use Catch instead of doctest"
    )
    parser.add_argument(
        "--disabled",
        action="store_true",
        help="DOCTEST_CONFIG_DISABLE / CATCH_CONFIG_DISABLE",
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        help="define the doctest/Catch fast config identifier",
    )
    parser.add_argument(
        "--files",
        type=int,
        default=1,
        help="number of source files (besides the implementation)",
    )
    parser.add_argument(
        "--tests",
        type=int,
        default=1,
        help="number of test cases per source file",
    )
    parser.add_argument(
        "--checks", type=int, default=1, help="number of asserts per test case"
    )
    parser.add_argument(
        "--asserts",
        choices=["normal", "binary"],
        default="normal",
...
parser_c.add_argument(
    "--implement",
    action="store_true",
    help="implement the framework test runner",
)
parser_c.add_argument(
    "--header",
    action="store_true",
    help="include the framework header everywhere",
)
...
parser_r.add_argument(
    ...
    type=int,
    default=1000,
    help="loop N times all asserts in each test case",
)
parser_r.add_argument(
    "--info", action="store_true", help="log the loop variable with INFO()"
)
...
if not os.path.exists("catch." + catch_ver + ".hpp"):
    urllib.request.urlretrieve(
        "https://github.com/catchorg/Catch2/releases/download/v"
        + catch_ver
        + "/catch.hpp",
        catch_header,
    )
...
if args.runtime or args.implement or args.header:
    if not args.catch:
        f.write("    int res = doctest::Context(argc, argv).run();\n")
    else:
        f.write("    int res = Catch::Session().run(argc, argv);\n")
else:
    f.write("    int res = 0;\n")
for i in range(0, args.files):
...
f.write("endif()\n\n")
if not args.catch:
    f.write('include_directories("../../../doctest/")\n\n')
else:
...
if args.compiler != "msvc":
    cmake_command = 'cmake . -G "MinGW Makefiles" -DCMAKE_BUILD_TYPE=' + (
        "Debug" if args.debug else "Release"
    )
if os.name != "nt":
    cmake_command = "cmake .                      -DCMAKE_BUILD_TYPE=" + (
        "Debug" if args.debug else "Release"
    )
...
if not args.runtime:
    print(
        "Time running compiler (+ linker) in seconds: "
...
if args.runtime:
    ...
    print(
        "Time running the tests in seconds: "
        + str((end - start).total_seconds())
    )
