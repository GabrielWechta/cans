...
class Account(object):
    ...
    def identity_keys(self):
        ...
        self._check_error(
            lib.olm_account_identity_keys(
                self._account, out_buffer, out_length
            )
        )
    ...
    def remove_one_time_keys(self, session):
        ...
        """
        self._check_error(
            lib.olm_remove_one_time_keys(self._account, session._session)
        )
    ...
    def fallback_key(self):
        ...
        keys.
        """
        out_length = lib.olm_account_unpublished_fallback_key_length(
            self._account
        )
        out_buffer = ffi.new("char[]", out_length)

        ret = lib.olm_account_unpublished_fallback_key(
            self._account, out_buffer, out_length
        )
