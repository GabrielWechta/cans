...
class InboundGroupSession(object):
    ...
    def _check_error(self, ret):
        ...
        last_error = bytes_to_native_str(
            ffi.string(lib.olm_inbound_group_session_last_error(self._session))
        )
    ...
    def id(self):
        ...
        ret = lib.olm_inbound_group_session_id(
            self._session, id_buffer, id_length
        )
        self._check_error(ret)
    ...
    def export_session(self, message_index):
        ...
        * OLM_UNKNOWN_MESSAGE_INDEX if we do not have a session key
        ...
        ret = lib.olm_export_inbound_group_session(
            self._session, export_buffer, export_length, message_index
        )
        self._check_error(ret)
        export_str = bytes_to_native_str(
            ffi.unpack(export_buffer, export_length)
        )
...
class OutboundGroupSession(object):
    ...
    def __new__(cls):
        ...
        track_for_finalization(
            obj, obj._session, _clear_outbound_group_session
        )
        return obj
    ...
    def session_key(self):
        ...
        ret = lib.olm_outbound_group_session_key(
            self._session, key_buffer, key_length
        )
        self._check_error(ret)
